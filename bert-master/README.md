...
python run_classifier.py \
  ...
  --output_dir=/tf_record/mrpc_output/
```
...
python run_squad.py \
  ...
  --max_seq_length=384 \
  --doc_stride=128 \
  --output_dir=/tf_record/squad_base/
```
...
# delimiter.
echo 'Who was Jim Henson ? ||| Jim Henson was a puppeteer' > /tf_record/input.txt

python extract_features.py \
  --input_file=/tf_record/input.txt \
  --output_file=/tf_record/output.jsonl \
  --vocab_file=$BERT_BASE_DIR/vocab.txt \
  --bert_config_file=$BERT_BASE_DIR/bert_config.json \
  --init_checkpoint=$BERT_BASE_DIR/bert_model.ckpt \
  --layers=-1,-2,-3,-4 \
...
python create_pretraining_data.py \
  ...
  --output_file=/tf_record/tf_examples.tfrecord \
  --vocab_file=$BERT_BASE_DIR/vocab.txt \
  --do_lower_case=True \
  --max_seq_length=128 \
...
python run_pretraining.py \
  --input_file=/tf_record/tf_examples.tfrecord \
  --output_dir=/tf_record/pretraining_output \
  --do_train=True \
  --do_eval=True \
  --bert_config_file=$BERT_BASE_DIR/bert_config.json \
  --init_checkpoint=$BERT_BASE_DIR/bert_model.ckpt \
