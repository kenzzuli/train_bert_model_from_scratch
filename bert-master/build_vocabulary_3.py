...
class VocabBuilder(object):
    ...
    def __init__(self):
        ...
        self.queue_for_all_dict_only = Manager().Queue()  # 用于存放总字典的序列，该序列仅存放一个总字典
    ...
    def join_dicts(self, tmp_count_dict):
        """
        将一个进程统计出来的字典与总字典合并
        :return:
        """
    ...
    def tokenize_count_join(self, file):
        """
        完整的一个流程，先分词，再统计，再字典合并
        :param file:
        :return:
        """
        self.join_dicts(self.count_token(self.tokenize_by_char(file)))
    ...
    def multi_process_token_counter(self):
        ...
        for file_path in self.file_full_path_list:
            # 往进程池中添加任务
            po.apply_async(self.tokenize_count_join, args=(file_path,))
