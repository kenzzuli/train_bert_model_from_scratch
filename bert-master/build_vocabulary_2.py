...
class VocabBuilder(object):
    ...
    def __init__(self):
        ...
        self.q_token_list_done = Manager().Queue()  # 用于存放已经完成频数统计的token_list的序列
    ...
    def token_counter(self):
        ...
        while True:
            ...
            self.q_token_list_done.put(1)  # 将完成频数统计的token_list放入序列，只放列表的长度即可，不用放全部数据
            if self.q_token_list_done.qsize() == len(self.file_full_path_list):
                break
    ...
    def multi_process_token_counter(self):
        po = Pool(processes=multiprocessing.cpu_count(), maxtasksperchild=10)
        # 占用内存不断增加，可能是因为处理不及时
